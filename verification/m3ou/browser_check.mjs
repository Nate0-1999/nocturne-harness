...
// Native acceptance covers pointer taps. Use accessible keyboard activation here
// so this deterministic API regression is independent of scaled-iframe coordinates.
const activate=async button=>{await wait(()=>button.isEnabled());await button.focus();await button.press('Enter')}
...
try{
 ...
 await activate(page.getByRole('button',{name:'Sheet',exact:true}))
 await conversation().getByTestId('composer').waitFor()
 await activate(conversation().getByRole('button',{name:'Out Loud',exact:true}))
 ...
 await activate(gate.getByTestId('memory-gate-continue'))
 ...
 assert.equal(await contentFrame().evaluate(()=>window.speechTest.recognitions.at(-1).active),false)
 await activate(conversation().getByRole('button',{name:'Read aloud',exact:true}))
 await activate(conversation().getByRole('button',{name:'Interrupt and listen',exact:true}))
 ...
 await activate(conversation().getByRole('button',{name:'Typing',exact:true}))
 assert.equal(await contentFrame().evaluate(()=>window.speechTest.recognitions.at(-1).active),false)
 await activate(conversation().getByRole('button',{name:'Out Loud',exact:true}))
 await page.reload()
 await activate(page.getByRole('button',{name:'Sheet',exact:true}))
 ...
 await activate(page.getByRole('button',{name:'Sheet',exact:true}))
 await page.locator('[data-rack-module="conversation"]').scrollIntoViewIfNeeded()
 await conversation().getByTestId('composer').scrollIntoViewIfNeeded()
 await page.mouse.move(0,0)
 await page.waitForTimeout(500)
 assert.equal(await page.locator('[data-rack-module="conversation"]').evaluate(el=>getComputedStyle(el).position),'relative')
 await activate(conversation().getByRole('button',{name:'Out Loud',exact:true}))
 await wait(()=>contentFrame().evaluate(()=>window.speechTest.recognitions.at(-1)?.active))
 await dictate('A phone voice draft')
 await wait(async()=>/A phone voice draft/.test(await conversation().getByTestId('composer').inputValue()))
 ...
 const result={speech_doubles:true,dictation_waits_for_send:true,send_uses_memory_gate:true,response_spoken:true,listening_after_response:true,typing_interrupts_dictation:true,control_interrupts_speech:true,preference_survives_reload:true,microphone_denied_explained:true,unsupported_keeps_typing:true}
 ...
 console.error(await contentFrame()?.evaluate(()=>({speech:window.speechTest,preferred:Object.keys(localStorage).filter(key=>key.startsWith('nocturne.out-loud:')).map(key=>localStorage.getItem(key)==='true')})))
 throw error
