...
const module = params.get('module') ?? 'roots'
const project = feed.projects.find((p) => p.root === params.get('root')) ?? feed.projects[0]
...
function Meter({ report }: { report: (value: string) => void }) {
  ...
  useFrame(({ camera }, delta) => {
    ...
    if (s.seconds >= 1 && s.samples.length < 12) {
      ...
      report(JSON.stringify({ module, tier, agents: feed.agents.length, fps: s.samples, minimum: Math.min(...s.samples), complete: s.samples.length === 12 }))
    }
  })
  return null
}
...
function App() {
  ...
  return <>
    <div style={{ position: 'fixed', top: 0, right: 0, background: '#b00', color: '#fff', font: '12px monospace', padding: '2px 8px' }}>FIXTURE · M3VL frame rate</div>
    ...
    <div style={{ width: 1230, height: 420 }}>
      <Canvas dpr={1} camera={{ fov: 42, position: [0, 2, 14] }} gl={{ antialias: tier === 'full' }}>
        ...
        {module === 'farm'
          ? <Farm project={project} agents={feed.agents} selectedId={null} selectedPath={null} tier={tier} asOf={feed.as_of} pick={() => {}} pickPath={() => {}} />
