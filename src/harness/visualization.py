...
def observe_worker(output: Path, assignment: dict, location, state: str) -> None:
    ...
    try:
        ...
        previous = {}
    value = {
        "id": assignment["origin_agent"], "thread_id": assignment["thread_id"],
        "parent_id": assignment["thread_id"], "root": str(location.workspace_root),
        "location": str(location.cwd), "workspace_root": str(location.workspace_root),
        "stage": assignment["stage"], "state": state,
    }
...
def work_observation(journal, home: Path, default_root: Path) -> dict:
    ...
    for entry in journal.catalog():
        ...
        agents.append({
            "id": entry.thread_id, "thread_id": entry.thread_id, "parent_id": None,
            "label": entry.title, "root": entry.workspace_root,
            "location": entry.current_location or entry.workspace_root,
            "state": state, "started_at": entry.created_at, "updated_at": entry.updated_at,
            "waiting_since": waiting_since, "cost_usd": None,
        })
    workers = {}
    for path in sorted((home / "symphonies").glob("**/visualization.json")):
        try:
            ...
            key = (f'{value["id"]}/{path.parent.name}'
                   if value["stage"] == "judge" else value["id"])
            value["id"] = key
            previous = workers.get(key)
            cost = None if meter.get("unpriced", True) else float(meter["cost_usd"])
            if previous is None:
                workers[key] = {**value, "started_at": value.get("started_at", value["ts"]),
                                "cost_usd": cost}
            else:
                started = min(previous["started_at"], value.get("started_at", value["ts"]))
                total = None if cost is None or previous["cost_usd"] is None else (
                    previous["cost_usd"] + cost
                )
    ...
    for value in workers.values():
        ...
        agents.append({**value, "label": value["id"].split("/")[-1],
                       "updated_at": value["ts"], "waiting_since": None})
        ...
        with sqlite3.connect(self.path) as db:
            ...
            if last is None or last[0] != digest:
                db.execute("INSERT INTO observation VALUES (?, ?, ?)",
                           (ts, digest, zlib.compress(encoded)))

    def read(self, as_of: str | None = None) -> dict:
        with sqlite3.connect(self.path) as db:
            timeline = [r[0] for r in db.execute("SELECT ts FROM observation ORDER BY ts")]
            rows = db.execute("SELECT ts, payload FROM observation WHERE (? IS NULL OR ts <= ?) "
                              "ORDER BY ts", (as_of, as_of)).fetchall()
        if not rows:
        ...
        for ts, blob in rows:
            ...
            for agent in value["agents"]:
                point = {"ts": ts, "location": agent["location"],
                         "cost_usd": agent["cost_usd"], "state": agent["state"]}
                points = trails.setdefault(agent["id"], [])
                if not points or any(points[-1][key] != point[key]
                                     for key in ("location", "cost_usd", "state")):
                    points.append(point)
        return {**value, "as_of": rows[-1][0], "live": as_of is None,
                "timeline": timeline, "recorded_since": timeline[0], "trails": trails}
...
def mount_visualization_routes(app: FastAPI, *, home, journal, root, graph_reader,
    ...
    async def sample():
        ...
        costs = {str(row["thread_id"]): row.get("total_usd")
