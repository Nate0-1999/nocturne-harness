const FEATURES = ['sem', 'kw', 'time', 'proj', 'freq', 'hist', 'loc', 'bias'] as const
    ...
    <div className="contribution-bars" aria-label="Weighted score contributions">
      {FEATURES.map((key) => {
        ...
        return <div className="contribution-row" key={key}>
          ...
          <output>{values[key] === null ? '—' : formatHumanScore(values[key] ?? 0)}</output>
        </div>
      })}
    </div>
  )
...
export function useContributionMap(): Record<string, Record<string, string | null>> {
  const { query } = useRackPlugin(); const rack = useRackSnapshot()
  const [values, setValues] = useState<Record<string, Record<string, string | null>>>({})
  useEffect(() => {
